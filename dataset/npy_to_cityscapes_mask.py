...
def load_road_masks(task_id: int, npys_dir: Path) -> NDArray[np.uint8]:
    ...
    road_paths = [
        p for p in task_paths if "road" in p.stem and "roadside" not in p.stem
    ]
...
def load_road_side_masks(task_id: int, npys_dir: Path) -> NDArray[np.uint8]:
    task_paths = npys_dir.glob(f"task-{task_id}*.npy")
    road_side_paths = [p for p in task_paths if "roadside" in p.stem]
...
def main() -> None:
    json_path = Path(
        "dataset/subset_789/json/project-1-at-2023-12-26-17-19-3d7c158e.json"
    )
    npys_dir = Path("dataset/subset_789/npys")
    output_dir = Path(
        "dataset/subset_789/masks"
    )
